...
Status: v2, incorporates BOTH loop reviews (design critique + engineering critique) as binding inputs. Approved for build 2026-09-23.
Grounding: JVM rig reproducing shipped v28d bytes (sha-verified), prod APK smali trace, MK's verbatim field outputs 2026-09-23.
v1 superseded; changes from v1 are marked [LOOP] and mapped to the originating critique in section 8.

## 1. Confirmed wrong-lock mechanism (unchanged from v1, rig-proven)

- k3 (English layout) cannot convert: byte-identical passthrough on MK's exact sentence. Hook fires only on k2 + voice flag; layout read live from the system InputMethod subtype per call (no stale-layout path).
- Neither field sentence locks on the rig as a clean full utterance; v28d rules behave as designed on clean ASR text.
- Firing mechanism: accent-garbled ASR emission (hwen, rigarding, profeshional, autputted, ting). Dictionary misses classify as strong Bangla; 3+ strong in a chain fires the v28d weight rule; the locked segment converts every trapped English word (the->থে, whole->ওহলে). Rig reproduction: "rigarding the rambler bangla project is it in the profeshional github repo" -> "ঋগারদিং থে রাম্বলের বাংলা project is it in the প্রফেশিওনাল গিথুব রেপো"; "outputted" -> ঔতপুত্তেদ (MK's exact field spelling); ড়াম্বলের implies a "drambler"-class emission.
- Intermittency = per-utterance ASR garble density. Clean emission: zero strong tokens, stays English.

## 2. Architecture (engineering-review minimum-safe set, adopted in full)
...
2. LocaleRouter - reads the pin ONLY. Never infers a route from first token, script, subtype, or lexical score. Weak/missing/conflicting session metadata = AUTO = v28d byte-identical behavior. [LOOP]
...
4. EvidenceNormalizer - builds NON-DESTRUCTIVE normalized evidence (scoring-only) after the ASR result is delivered, before classify/locks. Original text is never mutated for classification. [LOOP]
5. SegmentLock (v28d, byte-untouched verdict logic) - classifies on original tokens + scoring-only evidence. Dictionary hits NEVER seed a Bangla lock; a Tier-1 mapping is never a strong anchor. [LOOP]
...
- EN pin: PURE PASSTHROUGH. Returns the original string before any lookup, classification, or conversion - Tier-1 lookup explicitly suppressed. "A pin means do not touch my text." [LOOP]
- BN pin: k1-style convert-all PLUS the pre-existing guards: URL/email/@handle/code/number/time spans stay exact. [LOOP]
- AUTO (default, and the fallback for any weak/missing/conflicting metadata): v28d rules + A2 demotion (section 6). Byte-identical to v28d except the named A2 fixtures. [LOOP]
- Mixed-segment precedence: real Bangla strong tokens outweigh demoted garbles; a demoted token never blocks a lock that real Bangla strong evidence justifies, and never builds one. [LOOP]

## 4. Pin lifecycle (session-local, no persistence) [LOOP]
...
- Tier-1b: cmudict-derived (BSD) English loan spellings for uncovered high-frequency loans (732 uncovered top-1k words identified). Each shipped mapping carries provenance (source + derivation) and a regression fixture. [LOOP]
...
- Gates (all falsifiable, measured per corpus, not in aggregate): [LOOP]
...
## 7. Budgets, observability, kill gates (measured, release ceiling) [LOOP]
...
- Observability: counters and enums ONLY (lane chosen, demotions count, lookup hits count, pin resets by cause) + session nonce. NEVER raw dictated text, URLs, handles, or audio. [LOOP]
...
## 9. Amendment map (for the loop record)

| Amendment | Source | Landed in |
|---|---|---|
| Zero false demotions on ALL protected corpora, not aggregates | design review | sec 6 gates |
| Zero lock-DECISION changes except named fixtures; decisions not labels | design review | sec 6 gates, sec 8.4 |
| Per-token collision list published | design review | sec 6 gates |
| A2 = bounded ED1, ASCII alpha, len>=5, morphology+first/last constraints, garble allowlist | design review | sec 6 |
| Mixed-segment precedence: real Bangla strong outweighs demoted garbles | design review | sec 3 |
| NO acoustic locale routing in V29 (experiment, zero behavior dependency) | design review | v1 spike REMOVED; not in build |
| Pin lifecycle: session-local, reset events, never mid-dictation, no persistence | design + eng review | sec 4 |
| BN pin = k1 convert-all + URL/email/@handle/code/number/time guards | design review | sec 3 |
| EN pin = pure passthrough, Tier-1 suppressed | design review | sec 3, sec 8 |
| Tier-1 only in normalization lanes; names/handles/acronyms guarded first; provenance+fixture per mapping | design review | sec 5 |
| Budgets measured: <=1 MiB delta, <=4 MiB RSS, <=5ms p95 | design review | sec 7 |
| Counters/enums only, never raw text | design + eng review | sec 7 |
| Explicit mic-session routing; non-destructive normalization view; never infer route; weak metadata = v28d byte-identical | eng review | sec 2-3 |
| Protected spans on ORIGINAL first; normalized evidence scoring-only after ASR; originals retained | eng review | sec 2 |
| Canonical emission only unambiguous Latin rules, outside protected spans, only BN lane; never Bangla script; dictionary hit never seeds lock | eng review | sec 2, 5 |
| Component set (VoiceSessionContext/LocaleRouter/ProtectedSpan/EvidenceNormalizer/SegmentLock/Renderer + deterministic destruction) | eng review | sec 2 |
| Test matrices: Auto/BN/EN x locales x confidence; every lifecycle reset; garbles positive only where intended; mixed switches, one-token islands, two-word names, punctuationless, numbers/dates/times; differential fuzz; dictionary-shaped adversarial inserts; protected bytes/offsets exact | eng review | sec 8.3-4 + build battery |
| Kill gates (no auto-route if unobservable/nondeterministic; kill destructive dict on protected change/lock seed; kill pin on leak; rollback on floor regression) | eng review | sec 4, 7 |
| Hard question: gates before Tier-1 + all converter paths, byte-exact passthrough proof | design review | sec 8 |

## 10. Build discipline (unchanged)

Smallest patch on the v28d surface; v28d verdict logic byte-untouched; v27.11 baseline preserved. Full battery on the DEX-derived jar, CI smoke on the exact shipped bytes, signed prod+staging pair, SHA-256 + GoFile + provenance amendment. Repo stays PRIVATE; MK ships only after his own test pass.
